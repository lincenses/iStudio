...
namespace Studio.Configuration
{
    public class ConfigurationDocument
    {
        ...
        #region 获取具有指定键的配置组，如果不存在则在配置组集合中创建具有指定键的配置组并返回该配置组
        ...
        public string GetValue(string groupKey, string itemKey)
        {
            if (_ConfigurationGroupDictionary.ContainsKey(groupKey))
            {
                return _ConfigurationGroupDictionary[groupKey].GetValue(itemKey);
            }
            return "";
        }
        #endregion
